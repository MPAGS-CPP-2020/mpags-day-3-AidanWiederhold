//! Unit test for TransformChar.cpp
...
{
    /* Test all the punctuation characters I have access to on my keyboard,
    perhaps not quite as exhaustive as it should be so will look into a
    different way if I have time.*/
    std::string punc{"¬!\"£$%^&*()_+-=/-[{]}+;:'@#~\\|,<.>/?."};
    for(size_t i=0; i<punc.size(); i++)
    {
        REQUIRE(transformChar(punc[i])==std::string{""});
    }
}
